namespace VEthernet.Net.Tools
{
    ...
    // NOTE: Why implement IDictionary<TKey,TVal> interface? 
    // =============================================================
    // Problem
    // ----------
    ...
    // 
    // So there is no way without implement IDictionary<TKey,TVal>.
    //
    // Trade off
    // -------------
    // IDictionary<TKey,TVal> interface doesn't exist in .NET Framework v.4.0 or before.
    // In order to give priority to supporting serialization by JSON.NET, I had to truncate the support for .NET Framework 4.0.
    // (.NET Standard 1.4 support IDictionary<TKey,TVal>, therefore there is no problem on .NET Core appliction.)
    // 
    // Binary level compatiblity
    // -------------------------
    // There is no problem even if IPAddressRange is replaced with the latest version.
    // 
...
#else
    public sealed class IPAddressRange : IEnumerable<IPAddress>, IEquatable<IPAddressRange>
#endif
    {
...
# endif
        public IPAddressRange(IPAddress baseAddress, int maskLength)
        {
            ...
            if (baseAdrBytes.Length * 8 < maskLength)
                throw new FormatException(nameof(baseAddress));
...
# endif
        public bool Contains(IPAddress ipaddress)
        {
            ...
            var offset = 0;
            if (IsIPv4MappedToIPv6(Begin) && IsIPv4MappedToIPv6(ipaddress))
            {
...
# endif
        public static bool IsIPv4MappedToIPv6(IPAddress address)
        {
            ...
            unsafe
            {
                ushort* addressBuffer = stackalloc ushort[8];
                for (int i = addressBytes.Length / 2; i < 8; i++)
                {
                    addressBuffer[i] = 0;
                }

                Marshal.Copy(addressBytes, 0, (IntPtr)addressBuffer, addressBytes.Length);
                for (int i = 0; i < 5; i++)
                {
                ...
                return addressBuffer[5] == ushort.MaxValue;
            }
        }
...
# endif
        public bool Contains(IPAddressRange range)
        {
            ...
            var offset = 0;
            if (IsIPv4MappedToIPv6(Begin) && IsIPv4MappedToIPv6(range.Begin))
            {
                offset = 12; //ipv4 has prefix of 10 zero bytes and two 255 bytes. 
            }
...
# endif
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
# endif
        public static IPAddress SubnetMaskAddress(int cidr)
        {
            uint m = uint.MaxValue;
            m = CheckSum.htonl(m << (32 - cidr));
...
# endif
        ...
        {
            // Handle single IP
            if (IPFrame.Equals(Begin, End))
            {
                return 32;
            }
            uint x = CheckSum.ntohl(BitConverter.ToUInt32(Begin.GetAddressBytes(), 0));
            uint y = CheckSum.ntohl(BitConverter.ToUInt32(End.GetAddressBytes(), 0));
            uint m = y - x;
            for (int cidr = 31; cidr >= 0; cidr--)
            {
                int count = (1 << (32 - cidr)) - 1;
                if (count >= m)
                {
                    return cidr;
                }
...
# endif
        ...
        #region JSON.NET Support by implement IDictionary<string, string>
